...
// ── Intercept browser back button → go to coaching dashboard, not exam ──
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("leaderboardBtn").href =
    `/coaching/leaderboard.html?attempt=${attemptId}`;
...
async function calculateResult() {
  ...
  // Use time_taken saved by exam engine (accurate — measured from when student clicked Start)
  // Fall back to submitted_at - started_at only if time_taken was never recorded
  const timeTaken =
    attempt.time_taken != null
      ? Number(attempt.time_taken)
  ...
  if (attempt.total_score == null) {
    // Only update time_taken here if exam engine didn't save it (legacy/fallback)
    const updatePayload = { total_score: score, accuracy };
    if (attempt.time_taken == null) updatePayload.time_taken = timeTaken;
    await client
      .from("attempts")
...
window.shareResult = function () {
  ...
  const text = encodeURIComponent(
    `I just completed a Mock Test!\n\n` +
      `Score: ${score}  |  Correct: ${correct}  |  Accuracy: ${acc}\n\n` +
      `Powered by Courage Library: https://couragelibrary.in`,
  );
...
function showFatalError(msg) {
  document.getElementById("reviewSection").innerHTML = `
    <div style="text-align:center;padding:48px 20px;color:#64748b">
      ...
      <a href="/coaching/dashboard.html"
...
async function giveCoins(attemptId) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/functions/v1/submit-coaching-test-reward`,
      {
        method: "POST",
        headers: {
...
function showCoinPopup(coins, streak, breakdown) {
  ...
  popup.innerHTML = `
    ...
    ">
      💡 <strong style="color:#fbbf24">CL Coins</strong> work on
      <a href="https://couragelibrary.in" target="_blank"
        style="color:#93c5fd;text-decoration:underline;font-weight:700">Courage Library</a>
      — India's free all-India mock test platform. Join free to earn more &amp; compete nationally.
    </div>
    <div style="
      margin-top:14px;
      font-size:11px;
...
document.addEventListener('DOMContentLoaded', () => {
  // Coaching context: hide feedback trigger — not relevant for coaching exams
  const fbTrigger = document.getElementById('fbTriggerBtn');
  if (fbTrigger) fbTrigger.style.display = 'none';
  // fbCheckAlreadyDone and fbInitAutoPulse intentionally skipped in coaching context
});