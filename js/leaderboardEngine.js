...
async function lbLoad() {
  try {
    ...
    if (e1 || !attempt) { lbShowError(); return; }
    ...
    if (e2 || !attempts) { lbShowError(); return; }
...
function lbRenderAll(attempts) {
  ...
  if (myIdx >= 0) {
    ...
    const barPct = lbTopScore > 0 ? Math.round((me.total_score / lbTopScore) * 100) : 0;
...
function lbRenderList(source, rankMap) {
  ...
  if (!myInList && lbAttemptId) {
    ...
    if (me && myRank) {
      ...
      sep.style.cssText = 'text-align:center;font-size:.75rem;color:#94A3B8;padding:4px 0;letter-spacing:.08em;';
      sep.textContent = '· · · · ·';
...
function lbSetTab(btn, tab) {
  ...
  const src = q
    ? lbAttempts.filter(a => (a.user_profiles?.full_name || '').toLowerCase().includes(q))
    : lbAttempts;
  lbRenderList(src);
}
...
function lbShare() {
  ...
  if (navigator.share) {
    navigator.share({ title: 'My Exam Result — Courage Library', text, url: window.location.href })
      .catch(() => {});
  } else {
...
function lbSet(id, val) { const el = document.getElementById(id); if (el) el.textContent = val; }
function lbShow(id)     { const el = document.getElementById(id); if (el) el.style.display = ''; }
