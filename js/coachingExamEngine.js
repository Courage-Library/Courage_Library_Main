...
async function loadExam() {
  ...
  if (attemptCheck.user_id !== user.id) {
    ...
    setTimeout(() => { window.location.href = "/coaching/dashboard.html"; }, 2500);
    return;
  }

  if (attemptCheck.submitted_at) {
    showAlert("This exam is already submitted. Redirecting to your result…", "success");
    setTimeout(() => { window.location.href = "/coaching/result.html?attempt=" + attemptId; }, 1800);
    return;
  }
  ...
  const { data: attempt, error } = await client
    ...
    .select(`
      id,
      started_at,
      scheduled_exams!scheduled_exam_id(
        language,
        coaching_id,
  ...
  // Fetch coaching name for watermark (non-blocking)
  const coachingId = attempt.scheduled_exams?.coaching_id;
  if (coachingId) {
    client.from("coaching_centers").select("name").eq("id", coachingId).single()
      .then(({ data: cc }) => { if (cc?.name) window._coachingName = cc.name; })
      .catch(() => {});
  }
...
function generateWatermark() {
  ...
  // Use coaching name if loaded, fallback to Courage Library
  const brandName = window._coachingName || "Courage Library";
...
async function loadQuestions() {
  ...
  if (error) {
    ...
    if (container) container.innerHTML = `
      <div style="text-align:center;padding:40px 20px;color:#dc2626">
        ...
        <div style="font-weight:700;font-size:1rem;margin-bottom:6px">Could not load questions</div>
        <div style="font-size:.82rem;color:#94a3b8">${error.message}</div>
        <button onclick="location.reload()" style="margin-top:16px;padding:8px 20px;background:#1a56db;color:#fff;border:none;border-radius:10px;cursor:pointer;font-weight:700">Retry</button>
      </div>`;
    return;
  }

  // ── Guard: attempt_questions is empty ──
  if (!data || data.length === 0) {
    console.error("loadQuestions: attempt_questions is empty for attempt", attemptId);
    const container = document.getElementById("questionContainer");
    if (container) container.innerHTML = `
      <div style="text-align:center;padding:40px 20px;color:#92400e">
        <div style="font-size:2rem;margin-bottom:12px">📭</div>
        <div style="font-weight:700;font-size:1rem;margin-bottom:6px">No questions found</div>
        <div style="font-size:.82rem;color:#94a3b8">This exam has no questions assigned. Please contact your coaching admin.</div>
      </div>`;
    return;
  }

  // ── Map with null-safe section fallback ──
  // Priority: pattern_sections.section_name → questions.section_name → "General"
  const allQuestions = data
    .filter(d => d.questions !== null)
    .map((d) => ({
      ...d.questions,
      section: d.questions.pattern_sections?.section_name
             || d.questions.section_name
             || "General",
    }));
  ...
  if (allQuestions.length === 0) {
    ...
    if (container) container.innerHTML = `
      <div style="text-align:center;padding:40px 20px;color:#92400e">
        <div style="font-size:2rem;margin-bottom:12px">📭</div>
        <div style="font-weight:700;font-size:1rem;margin-bottom:6px">Questions could not be loaded</div>
        <div style="font-size:.82rem;color:#94a3b8">The questions may have been deleted. Contact your coaching admin.</div>
      </div>`;
    return;
  }
...
async function submitExam() {
  ...
  try {
    // Step 1 — calculate actual time taken from when student clicked "Start"
    // sessionStorage key is set in the rules modal "Start" button handler
    const actualStartKey = "examActualStart_" + attemptId;
    const storedStart = sessionStorage.getItem(actualStartKey);
    let timeTaken;
    if (storedStart) {
      timeTaken = Math.floor((Date.now() - Number(storedStart)) / 1000);
      sessionStorage.removeItem(actualStartKey);
    } else {
      // Fallback: fetch started_at from DB (less accurate but better than wrong)
      const { data: attemptData } = await withRetry(() =>
        client.from("attempts").select("started_at").eq("id", attemptId).single()
      );
      timeTaken = Math.floor((Date.now() - Date.parse(attemptData.started_at)) / 1000);
    }
    ...
    window.location.href = `/coaching/result.html?attempt=${attemptId}`;
    ...
    <div style="background:#fff;border-radius:20px;max-width:380px;width:100%;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.3)">
      ...
      <div style="padding:20px 24px">
        ...
        <div style="display:flex;flex-direction:column;gap:10px">
          ...
          <a href="/coaching/dashboard.html" style="display:block;text-align:center;padding:10px;border-radius:12px;border:1.5px solid #e2e8f4;color:#64748b;font-size:.82rem;font-weight:600;text-decoration:none;font-family:inherit">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  startBtn.addEventListener("click", () => {
    ...
    // Record the actual moment the student starts the exam (not attempt creation time)
    sessionStorage.setItem("examActualStart_" + (new URLSearchParams(window.location.search).get("attempt")), String(Date.now()));
